...
export default function BlogIndexClient({ initialPosts }: Props) {
  ...
  const visible = published.filter((p) => !showUnreadOnly || !readPosts[p.slug]);
    ...
    <div style={{ width: "100%", minHeight: "100vh", background: t.paper, color: t.ink, fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ width: "100%", maxWidth: 1280, position: "relative" }}>
        ...
        <div style={{ padding: "80px 80px 40px", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          ...
          {isSignedIn && (
            <div style={{ display: "flex", alignItems: "center", gap: 12, background: t.paperAlt, padding: "12px 20px", borderRadius: 30, border: `1px solid ${t.rule}`, boxShadow: theme === "dark" ? "0 4px 12px rgba(0,0,0,0.4)" : "0 4px 12px rgba(0,0,0,0.05)" }}>
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: 13, fontWeight: 500, color: t.inkSoft }}>Hide read articles</span>
              <button
                onClick={() => setShowUnreadOnly(!showUnreadOnly)}
                style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: showUnreadOnly ? t.accent : t.rule, position: "relative", cursor: "pointer", transition: "background 0.3s" }}
              >
                <motion.div
                  initial={false}
                  animate={{ x: showUnreadOnly ? 20 : 2 }}
                  style={{ width: 20, height: 20, borderRadius: 10, background: "#fff", position: "absolute", top: 2, boxShadow: "0 2px 4px rgba(0,0,0,0.2)" }}
                />
              </button>
            </div>
          )}
              ...
              return (
                ...
                >
                  <Link href={`/blog/${post.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block", height: "100%" }} className="group">
                    <div
                      style={{ height: "100%", background: t.paperAlt, border: `1px solid ${t.rule}`, borderRadius: 12, padding: 40, position: "relative", overflow: "hidden", transition: "transform 0.3s, box-shadow 0.3s", opacity: isRead && !showUnreadOnly ? 0.6 : 1 }}
                      className={`group-hover:-translate-y-1 group-hover:shadow-xl ${theme === "dark" ? "group-hover:shadow-white/5" : ""}`}
                    >
                      <div style={{ position: "absolute", top: -50, right: -50, width: 200, height: 200, background: pal.bg, borderRadius: "50%", filter: "blur(50px)", opacity: theme === "dark" ? 0.15 : 0.6, zIndex: 0, pointerEvents: "none", transition: "opacity 0.3s" }} className="group-hover:opacity-100" />

                      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                            {post.tags.map((tag) => (
                              <span key={tag} style={{ fontSize: 10, fontFamily: "JetBrains Mono", color: pal.text, background: pal.bg, padding: "4px 8px", borderRadius: 4, textTransform: "uppercase", fontWeight: 600 }}>
                                {tag}
                              </span>
                            ))}
                          </div>
                          {isSignedIn && (
                            <button
                              ...
                              style={{ background: isRead ? "#7aa86b" : "transparent", border: `1px solid ${isRead ? "#7aa86b" : t.rule}`, color: isRead ? "#fff" : t.inkMuted, width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.2s", zIndex: 10 }}
