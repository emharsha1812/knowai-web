...
export default function CoursesIndex() {
  ...
  useEffect(() => {
    getCourses()
      .then(data => setCourses(data.filter(c => c.is_published)))
      ...
      <div style={{ width: '100%', maxWidth: 1280, position: 'relative' }}>
        ...
        <div style={{ padding: '80px 80px 40px', maxWidth: 800 }}>
          ...
          <p style={{ fontFamily: 'Newsreader, serif', fontSize: 20, color: t.inkSoft, lineHeight: 1.5 }}>
            Comprehensive, multi-week programs designed to build intuition from the ground up. Interactive syllabi with integrated notebooks.
          </p>
        </div>
        ...
        <div style={{ padding: '20px 80px 100px', display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          {!loading && !error && courses.length === 0 && (
            <div style={{ textAlign: 'center', padding: '80px 0', color: t.inkMuted }}>
              <Mono theme={theme}>No courses yet.</Mono>
            </div>
          )}
                ...
                <Link href={`/courses/${course.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }} className="group">
                  ...
                  >
                    <div style={{
                      ...
                      background: pal.bg, borderRadius: '50%', filter: 'blur(60px)', opacity: theme === 'dark' ? 0.15 : 0.6,
                    ...
                    <div style={{
                      ...
                      <div style={{ marginTop: 12 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: t.accentInk, letterSpacing: '0.05em', background: t.accent, padding: '10px 20px', borderRadius: 4, display: 'inline-block', transition: 'all 0.3s' }}>
                          VIEW SYLLABUS →
                        </span>
                      </div>
                    </div>
